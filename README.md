...
* `com.amazon.corretto.crypto.provider.registerEdKeyFactory`
  ...
  such as Signature.
* `TARGET_JDK_VERSION` 
  Takes in an integer to set the minimum JDK version for the build to target and release on (21/17/11/8). Releases on JDK 8 by default if nothing is passed for `TARGET_MIN_JDK.`
  On JDK 17 and 21+, the KEM API is supported so a `jdk17plus` directory will be included on compilation to add post quantum support with ML-KEM. 
