...
class EvpHmac extends MacSpi implements Cloneable {
    /**
     * When passed to {@code evpMd} indicates that the native code should not call
     * HMAC_Init_ex.
     */
    ...
    /**
     * Calls {@code HMAC_Update} with {@code input}, possibly calling {@code HMAC_Init_ex} first
     * (if {@code evpMd} is any value except {@link #DO_NOT_INIT}).
     * This method should only be used via {@link #synchronizedUpdateCtxArray(byte[], byte[], long, byte[], int, int)}.
     *
     * @param ctx opaque array containing native context
     */
    private static native void updateCtxArray(byte[] ctx, byte[] key, long evpMd, byte[] input, int offset, int length);
    /** @see {@link #updateCtxArray(byte[], byte[], long, byte[], int, int)} */
    private static void synchronizedUpdateCtxArray(byte[] ctx, byte[] key, long evpMd, byte[] input, int offset, int length) {
        synchronized(ctx) {
    ...
    /**
     * Calls {@code HMAC_Final}, and places the result in {@code result}.
     * This method should only be called via {@link #synchronizedDoFinal(byte[], byte[])}
     *
     * @param ctx  opaque array containing native context
     * @param result
     */
    ...
    /**
     * Calls {@code HMAC_Init_ex}, {@code HMAC_Update}, and {@code HMAC_Final} with
     * {@code input}.
     * This method should only be used via
     * {@link #synchronizedFastHmac(byte[], byte[], long, byte[], int, int, byte[])}.
     *
     * @param ctx opaque array containing native context
     */
    ...
    private static class TestMacProvider extends Provider {
        ...
        public synchronized Service getService(final String type, final String algorithm) {
            if (type.equals("Mac") && algorithm.equals(macName)) {
                return new Service(this, type, algorithm, spi.getName(), Collections.emptyList(),
        ...
        try (final Scanner in = new Scanner(Loader.getTestData("hmac.txt"), StandardCharsets.US_ASCII.name())) {
            ...
            while (in.hasNext()) {
                ...
                if (type.equals(hashCategory.get(macName))) {
                    Utils.testMac(testMac, key, message, Utils.decodeHex(expecteds[hashLocation.get(macName)]));
                }
            }
