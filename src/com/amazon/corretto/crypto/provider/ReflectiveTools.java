...
final class ReflectiveTools {
  ...
  static void enableByteBufferReflection() throws NoSuchFieldException, IllegalAccessException {
    ...
    mh_getArray =
        LOOKUP
            .findGetter(ByteBuffer.class, "hb", byte[].class)
            .asType(MethodType.methodType(byte[].class, ByteBuffer.class));
    mh_arrayOffset =
        LOOKUP
            .findGetter(ByteBuffer.class, "offset", Integer.TYPE)
            .asType(MethodType.methodType(Integer.class, ByteBuffer.class));
  }

  static void disableByteBufferReflection() {
    // If we failed to do our reflective hackery, just disable all this sneaky stuff by always
    // returning null
    // from mh_getArray.
    mh_getArray =
        MethodHandles.dropArguments(
            MethodHandles.constant(byte[].class, null), 0, ByteBuffer.class);
  }

  /**
   * Obtains an array from a ByteBuffer. This allows us to access the byte[] backing a read-only
   * byte buffer, by reflecting into the private array field.
   */
