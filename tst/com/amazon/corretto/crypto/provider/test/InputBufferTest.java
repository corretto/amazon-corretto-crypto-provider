...
public class InputBufferTest {
  ...
  private <T, S> InputBuffer<T, S, RuntimeException> getBuffer(int capacity) {
    try {
      return (InputBuffer<T, S, RuntimeException>)
          sneakyConstruct(InputBuffer.class.getName(), capacity);
  ...
  public void requiresPositiveCapacity() throws Throwable {
    assertThrows(
        ...
        () -> sneakyConstruct(InputBuffer.class.getName(), Integer.valueOf(-1)));
  ...
  public void cloneDuplicatesBufferAndState() throws Throwable {
    ...
    buffer1
        .withInitialStateSupplier((s) -> new ByteArrayOutputStream())
        .withUpdater(
            (state, src, offset, length) -> {
    ...
    final InputBuffer<byte[], ByteArrayOutputStream, RuntimeException> buffer2 =
        (InputBuffer<byte[], ByteArrayOutputStream, RuntimeException>)
            TestUtil.sneakyInvoke(buffer1, "clone");
    buffer1.update(expected, 0, expected.length);
    buffer1.update(expected, 0, expected.length);
    ...
    assertArrayEquals(
        expected,
        Arrays.copyOfRange(buff1Result, idx, idx + expected.length),
        "Buff 1 first quarter");
    idx += expected.length;
    assertArrayEquals(
        expected,
        Arrays.copyOfRange(buff1Result, idx, idx + expected.length),
        "Buff 1 second quarter");
    idx += expected.length;
    assertArrayEquals(
        expected,
        Arrays.copyOfRange(buff1Result, idx, idx + expected.length),
        "Buff 1 third quarter");
    idx += expected.length;
    assertArrayEquals(
        expected,
        Arrays.copyOfRange(buff1Result, idx, idx + expected.length),
        "Buff 1 forth quarter");

    byte[] buff2Result = buffer2.doFinal();
    idx = 0;
    assertEquals(expected.length * 2, buff2Result.length, "Buff 2 result length");
    assertArrayEquals(
        expected, Arrays.copyOfRange(buff2Result, idx, idx + expected.length), "Buff 2 first half");
    idx += expected.length;
    assertArrayEquals(
        expected,
        Arrays.copyOfRange(buff2Result, idx, idx + expected.length),
        "Buff 2 second half");
  }
  ...
  public void cantCloneUncloneable() throws Throwable {
    ...
    TestUtil.assertThrows(
        ...
        () -> {
          TestUtil.sneakyInvoke(buffer, "clone");
        });
  }
