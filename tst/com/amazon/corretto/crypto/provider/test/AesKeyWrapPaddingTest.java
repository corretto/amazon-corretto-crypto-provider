...
public final class AesKeyWrapPaddingTest {
    ...
    public void testEngineGetParametersAndIv() throws Exception {
        ...
        final int[] modes = new int[] {Cipher.ENCRYPT_MODE, Cipher.ENCRYPT_MODE, Cipher.ENCRYPT_MODE, Cipher.ENCRYPT_MODE};
        for (int mode : modes) {
    ...
    @ParameterizedTest
    @MethodSource("getParamsIncremental")
    public void testUpdateDoFinalOverloads(int wrappingKeySize, int secretSize, int stepSize, int doFinalSize)
            throws Exception {
    ...
    public void testUpdateWithOversizedInputLen() throws Exception {
        ...
        assertThrows(IllegalArgumentException.class, () -> c.update(plaintext, 0, plaintext.length+1));
        assertThrows(IllegalArgumentException.class, () -> c.update(plaintext, 1, plaintext.length));
        assertThrows(IllegalArgumentException.class, () -> c.update(plaintext, plaintext.length+1, 0));
        c.update(plaintext, 0, plaintext.length);
        final byte[] ciphertext = c.doFinal();
        c.init(Cipher.DECRYPT_MODE, kek);
        assertThrows(IllegalArgumentException.class, () -> c.doFinal(ciphertext, 0, ciphertext.length+1));
        assertThrows(IllegalArgumentException.class, () -> c.doFinal(ciphertext, 1, ciphertext.length));
        assertThrows(IllegalArgumentException.class, () -> c.doFinal(ciphertext, ciphertext.length+1, 0));
    ...
    public void testBadInputs_setModeAndPadding() throws Throwable {
        ...
        assertThrows(
            GeneralSecurityException.class,
            () -> sneakyInvoke(spi, "engineSetMode", "OTHER")
        );
        assertThrows(
            GeneralSecurityException.class,
            () -> sneakyInvoke(spi, "engineSetPadding", "Other")
        );
    ...
    public void testBadInputs_getBlockAndKeySize() throws Throwable {
        ...
        assertThrows(
            ...
            () -> sneakyInvoke(spi, "engineGetKeySize", NULL_KEY)
        );
    ...
    public void testBadInputs_doFinal() throws Exception {
        ...
        assertThrows(ShortBufferException.class, () -> c.doFinal(new byte[1], 0, 1, new byte[dataSize+1], 0));
    }
