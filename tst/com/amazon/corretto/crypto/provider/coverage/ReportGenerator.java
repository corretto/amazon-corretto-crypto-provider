...
public class ReportGenerator {
  ...
  private static void generateReports(
      ...
      final String classDirName,
      final String srcDirName,
      final File resultHtmlDir,
      final File xmlFile)
    ...
    try (final FileOutputStream xmlOut = new FileOutputStream(xmlFile)) {
      ...
      visitor.visitInfo(
          execLoader.getSessionInfoStore().getInfos(),
          execLoader.getExecutionDataStore().getContents());
  ...
  private static void generateBrazilCoverage(final File xmlReport, final File resultDir)
      throws Exception {
    ...
    final Pattern counterPattern =
        Pattern.compile(
            "<counter\\s+type=\"([^\"]+)\"\\s+missed=\"(\\d+)\"\\s+covered=\"(\\d+)\"/>");
